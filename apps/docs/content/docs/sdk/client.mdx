...
|--------|------|-------------|
| `companyId` | `number` | ID de l'entreprise (obligatoire) |
| `tokens` | `AuthTokens` | Tokens d'authentification (optionnel, utilise le fichier local par défaut) |
| `tokenManager` | `TokenManager` | Instance custom de TokenManager (optionnel) |

<Callout type="info">
  Par défaut, le client utilise le token et le `companyId` stockés localement par `tiime auth login` et `tiime company use`. Vous pouvez les surcharger via les options.
</Callout>
...
```ts title="errors.ts"
import { TiimeClient, TiimeError } from "tiime-cli";
